...
public class ChunkToProtochunkFix extends DataFix {
   ...
   private static <T> Dynamic<T> fixChunkData(Dynamic<T> p_199856_) {
      boolean flag = p_199856_.get("TerrainPopulated").asBoolean(false) && (p_199856_.get("LightPopulated").asNumber().result().isEmpty() || p_199856_.get("LightPopulated").asBoolean(false));
      Dynamic<T> dynamic;
      if (flag) {
         dynamic = repackTicks(repackBiomes(p_199856_));
      } else {
         dynamic = createEmptyChunk(p_199856_);
      }

      return dynamic.set("Status", p_199856_.createString(flag ? "mobs_spawned" : "empty")).set("hasLegacyStructureData", p_199856_.createBoolean(true));
   }
   ...
   private static <T> Dynamic<T> createEmptyChunk(Dynamic<T> p_199884_) {
      Builder<Dynamic<T>, Dynamic<T>> builder = ImmutableMap.builder();
      p_199884_.get("xPos").result().ifPresent((p_199878_) -> {
         builder.put(p_199884_.createString("xPos"), p_199878_);
      });
      p_199884_.get("zPos").result().ifPresent((p_199854_) -> {
         builder.put(p_199884_.createString("zPos"), p_199854_);
      });
      return p_199884_.createMap(builder.build());
   }
